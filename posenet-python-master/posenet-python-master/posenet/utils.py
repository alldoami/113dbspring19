...
def draw_skel_and_kp(
        img, instance_scores, keypoint_scores, keypoint_coords,maxima_found,
    ...
    out_img = cv2.drawKeypoints(
        out_img, cv_keypoints, outImage=np.array([]), color=color_out,
        flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    out_img = cv2.polylines(out_img, adjacent_keypoints, isClosed=False, color=color_out)
    return out_img
